...
- **Multi-Document Support for `--with-doc`**: Enhanced the `ask`, `plan`, `repo`, and `doc` commands to accept multiple `--with-doc=<url>` flags. This allows providing context from several external documents simultaneously.
