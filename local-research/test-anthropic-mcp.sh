...
# Test script for MCP integration with Anthropic provider
# Tests various MCP servers with Anthropic
...
run_test() {
  ...
  if [ ! -z "$model" ]; then
    ...
    echo "Running: pnpm dev mcp run \"$query\" --provider=anthropic $model_arg"
  else
    echo "Running: pnpm dev mcp run \"$query\" --provider=anthropic"
  fi
  
  if [ ! -z "$model" ]; then
    pnpm dev mcp run "$query" --provider=anthropic --model=$model
  else
    pnpm dev mcp run "$query" --provider=anthropic
  fi
...
# Main test execution
echo "Starting MCP integration tests with Anthropic provider..."
...
run_test "Filesystem MCP Server - List JavaScript files" "List all JavaScript files in this directory" "claude-3-opus-20240229"
...
echo "------------------------"
echo "All Anthropic tests completed." 